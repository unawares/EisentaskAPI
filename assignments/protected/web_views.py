...
class Utils:
    ...
    def get_active_tasks_instance(assignment_info):
        ...
        archived_task_ids = Utils._get_all_archived_tasks_instance(assignment_info).values_list('assignment_task', flat=True)
        completed_task_ids = Utils._get_all_completed_tasks_instance(assignment_info).values_list('assignment_task', flat=True)
        return Utils._get_all_tasks_instance(assignment_info).exclude(id__in=archived_task_ids).exclude(id__in=completed_task_ids)

    @staticmethod
    def get_completed_tasks_instance(assignment_info):
        archived_task_ids = Utils._get_all_archived_tasks_instance(assignment_info).values_list('assignment_task', flat=True)
        complete_task_ids = Utils._get_all_completed_tasks_instance(assignment_info).exclude(assignment_task__in=archived_task_ids).values_list('assignment_task', flat=True)
...
def complete_assigned_task(request, uuid, info, pk):
    ...
    return redirect(assignment_info)
